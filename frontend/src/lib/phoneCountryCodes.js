/** ISO 3166-1 alpha-2 + ITU calling codes (primary). */
...
export function getDefaultPhonePrefix(lang = "en") {
  return lang === "fr" ? "+33" : "+1";
}

export function getDefaultPhoneCountryIso2(lang = "en") {
  return lang === "fr" ? "FR" : "US";
}

export function findCountryByIso2(iso2) {
  const code = String(iso2 || "").toUpperCase();
  return PHONE_COUNTRIES.find((country) => country.iso2 === code) || null;
}

export function resolveCountryByDial(dial, lang = "en") {
  const normalized = String(dial || "").trim();
  const matches = PHONE_COUNTRIES.filter((country) => country.dial === normalized);
  if (!matches.length) return null;
  if (normalized === "+1") {
    const preferred = lang === "fr" ? "CA" : "US";
    return matches.find((country) => country.iso2 === preferred) || matches[0];
  }
  return matches[0];
}
...
export function filterPhoneCountries(query, lang = "en") {
  ...
  return withLabels.filter((country) => {
