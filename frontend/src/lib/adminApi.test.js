...
describe("isTransientNetworkError", () => {
  it("detects axios network failures but not timeouts", () => {
    ...
    expect(
      isTransientNetworkError({ code: "ECONNABORTED", message: "timeout of 15000ms exceeded" }),
    ).toBe(false);
...
describe("isRequestTimeoutError", () => {
  it("detects axios timeouts", () => {
    expect(
      isRequestTimeoutError({ code: "ECONNABORTED", message: "timeout of 15000ms exceeded" }),
    ).toBe(true);
...
describe("adminApiErrorMessage", () => {
  it("explains gateway timeouts", () => {
    const msg = adminApiErrorMessage(
      { response: { status: 504, data: "timeout" } },
      "Execution failed",
    );
    ...
    const msg = adminApiErrorMessage(
      {
        response: {
          status: 500,
          data: { detail: { message: "Proxy could not reach target host (HTTP 572)." } },
        },
      },
      "Execution failed",
    );
  ...
  it("explains client timeouts", () => {
    const msg = adminApiErrorMessage(
      { code: "ECONNABORTED", message: "timeout of 480000ms exceeded" },
      "Execution failed",
    );
