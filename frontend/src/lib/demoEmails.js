...
function messagesForApplication(app) {
  ...
  const submitted = app.submission_status === "submitted"
...
function staticShowcaseMessages(apps) {
  const byCompany = (needle) => apps.find((app) =>
    String(app.job?.company || "").toLowerCase().includes(needle),
  );
  ...
  if (!sg) {
    extras.push(buildMessage({
      id: "demo_static_interview_sg",
      application: {
        application_id: "demo_static_sg",
        job_id: "demo_static_sg",
        job: { company: "Société Générale", title: "Analyste Risques de Marché", job_id: "demo_static_sg" },
        status: "interview",
  ...
  if (!natixis) {
    extras.push(buildMessage({
      ...
      preview: "Congratulations! We are pleased to extend an offer for the Quantitative Analyst role.",
  ...
  if (!cacib) {
    extras.push(buildMessage({
      ...
      application: {
        ...
        job: { company: "Crédit Agricole CIB", title: "Analyste DCM", job_id: "demo_static_cacib" },
        status: "applied",
