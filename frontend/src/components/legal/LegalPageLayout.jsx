...
export default function LegalPageLayout({ title, description, canonical, lastUpdated, children }) {
  const navigate = useNavigate();
  const onApp = isAppHost();

  return (
    <div className="min-h-dvh bg-white text-zinc-900">
      <SEOHead title={`${title} | ${BRAND.NAME}`} description={description} canonical={canonical} />
