...
export default function JobCardHighlights({ job, t, lang, max = 3, theme, compact = false }) {
    ...
    <div className={wrapClass} data-testid="job-card-highlights">
      <dl className={compact ? "space-y-1.5" : "space-y-2"}>
        {rows.map((row) => (
          <div
            key={row.key}
            className="grid grid-cols-[minmax(0,38%)_1fr] gap-x-2 gap-y-0.5 text-left sm:grid-cols-[minmax(0,34%)_1fr]"
          >
            <dt
              className={`text-[10px] font-semibold uppercase tracking-wide sm:text-[11px] ${labelClass}`}
            >
              {row.label}
            </dt>
            <dd className={`text-[11px] leading-snug sm:text-xs ${valueClass}`}>{row.value}</dd>
          </div>
        ))}
      </dl>
    </div>
