...
const copyFor = (state) => {
  switch (state) {
    case "exhausted":
      return ["swipe.feedExhaustedTitle", "swipe.feedExhaustedBody"];
    case "policy_hidden":
      return ["swipe.feedPolicyHiddenTitle", "swipe.feedPolicyHiddenBody"];
    case "blocked":
      return ["swipe.feedBlockedTitle", "swipe.feedBlockedBody"];
    case "no_inventory":
      return ["swipe.feedNoInventoryTitle", "swipe.feedNoInventoryBody"];
    case "profile_not_ready":
      return ["swipe.feedProfileNotReadyTitle", "swipe.feedProfileNotReadyBody"];
    default:
      return ["swipe.noJobs", "swipe.tryWidenSearch"];
  }
};
...
export default function SwipeFeedTerminalState({
  ...
  return (
    <section
      ...
      role="status"
      aria-live="polite"
      data-testid={`feed-terminal-${state}`}
    >
      ...
      <div className="mt-6 flex flex-wrap justify-center gap-2">
        {actions.map(({ id }) => {
          if (id === "revisit_later") {
            return (
              <p key={id} className="text-sm text-sprout-muted">
                {t("swipe.feedSuggestionRevisitLater")}
              </p>
            );
          }
          ...
          return (
            <button
              ...
              className="inline-flex min-h-11 items-center gap-2 rounded-full border border-sprout-border bg-sprout-surface px-4 text-sm font-semibold text-sprout-text hover:border-sprout-mint"
