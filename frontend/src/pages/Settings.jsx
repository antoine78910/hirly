...
import { countEnabledNotifications, readNotificationSettings } from "../lib/notificationSettings";
...
const Row = ({ icon: Icon, label, value, onClick, danger, testId }) => (
  ...
  >
    {Icon && (
      <Icon className={`w-5 h-5 ${danger ? "text-rose-400" : "text-white"}`} strokeWidth={1.9} />
    )}
    <span className={`flex-1 text-[15px] font-medium ${danger ? "text-rose-400" : "text-white"}`}>
      {label}
    </span>
...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("hirly:notification-settings-changed", syncNotifications);
  }, []);
  ...
  return (
    <>
      ...
      <AppPage className="sprout bg-sprout-bg text-white md:hidden">
        <header
          className="mx-auto w-full max-w-md shrink-0 px-5 pt-6 flex items-center gap-3"
        ...
        >
          ...
          <h1 className="font-display font-bold text-2xl flex-1 text-center pr-10">
            {t("settings.title")}
          </h1>
        </header>
        ...
        <AppPageScroll className="mx-auto max-w-md px-5 pb-32" withBottomNavPad={false}>
          <Section label={t("settings.lookAndFeel")} testId="settings-appearance">
            <Row
              ...
              value={mobileThemeLabel(theme, t)}
          ...
          <Section label={t("settings.planHelp")} testId="settings-support">
            <Row
              ...
              value={
                billing?.is_premium
                  ? formatPlanTier(billing?.plan_tier || billing?.plan) || undefined
                  : undefined
              }
              onClick={openBilling}
            ...
            <Row
              icon={MessageSquare}
              label={t("settings.talkToUs")}
              onClick={() => setContactOpen(true)}
              testId="settings-chat"
            />
          </Section>
          ...
          <Section label={t("settings.shareBrand", { brand: BRAND.NAME })} testId="settings-social">
            <Row
              ...
              value={t("settings.inviteFriendHint")}
              onClick={() => navigate("/referral")}
              testId="settings-invite"
            />
            <Row
              icon={Instagram}
              label={t("settings.onInstagram")}
              value={t("settings.socialHandle")}
              onClick={() => openExternal(BRAND.INSTAGRAM_URL)}
              testId="settings-instagram"
            />
            <Row
              icon={TikTok}
              label={t("settings.onTikTok")}
              value={t("settings.socialHandle")}
              onClick={() => openExternal(BRAND.TIKTOK_URL)}
          ...
          <footer
            className="mt-10 mb-4 flex flex-col items-center gap-2 text-center"
          ...
          >
            ...
            <code
              className="text-[11px] text-sprout-dim break-all px-3"
