        ...
        <div className='mt-10 w-full'>
          ...
          {mode === 'request' ? (
            <form className='mt-8 space-y-3' onSubmit={requestReset}>
              ...
              <Button type='submit' disabled={submitting} className='h-12 w-full rounded-full gradient-linkedin font-semibold text-white hover:opacity-90'>
            ...
            <form className='mt-8 space-y-3' onSubmit={updatePassword}>
              <label className='block'>
                ...
                <div className='relative'>
                  ...
                  <Input type='password' value={password} onChange={(event) => setPassword(event.target.value)} placeholder={copy.password} autoComplete='new-password' className='h-12 rounded-2xl pl-10' />
                </div>
              </label>
              <label className='block'>
                ...
                <div className='relative'>
                  <Lock className='pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-400' />
                  <Input type='password' value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder={copy.confirmPassword} autoComplete='new-password' className='h-12 rounded-2xl pl-10' />
                </div>
              </label>
              {error ? <p className='text-sm font-medium text-red-600'>{error}</p> : null}
              <Button type='submit' disabled={submitting} className='h-12 w-full rounded-full gradient-linkedin font-semibold text-white hover:opacity-90'>
          ...
          {mode !== 'loading' && mode !== 'update' ? (
            <p className='mt-6 text-center text-sm'>
              <Link to='/signin' className='font-semibold text-linkedin hover:text-linkedin-dark'>{copy.back}</Link>
            </p>
          ) : null}
        </div>
      </div>
