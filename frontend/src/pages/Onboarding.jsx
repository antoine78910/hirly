...
export default function Onboarding() {
  ...
  const { user, hasProfile, hasPreferences, loading: authLoading, setHasProfile, setHasPreferences } = useAuth();
  ...
  useEffect(() => {
    console.log("ONBOARDING_AUTH_STATE", {
      user: Boolean(user),
      hasProfile,
      hasPreferences,
      authLoading,
  ...
  }, [authLoading, hasPreferences, hasProfile, step, user]);

  useEffect(() => {
    if (authLoading || !user || step !== "signup") return;
    setStepIndex(STEP_ORDER.indexOf("jobSearch"));
  }, [authLoading, step, user]);
  ...
  const handleUpload = async (f) => {
    ...
    setParsing(true);
    try {
      console.log("ONBOARDING_CV_UPLOAD_START", {
        filename: f.name,
        size: f.size,
        type: f.type,
      });
      ...
      console.log("ONBOARDING_CV_UPLOAD_RESPONSE", data);
      setFile(f);
      setProfile(data);
      const { data: authState } = await api.get("/auth/me");
      setHasProfile(Boolean(authState?.has_profile));
      setHasPreferences(Boolean(authState?.has_preferences));
      if (!authState?.has_profile) {
        toast.error("Your CV upload did not finish saving. Try uploading it again.");
        return;
      }
  ...
  const persistOnboardingMeta = async () => {
    try {
      const { data } = await api.patch("/profile/extras", {
        onboarding: {
      ...
      console.log("ONBOARDING_SAVE_PROFILE_RESPONSE", data);
    } catch (e) {
      console.warn("extras save skipped", e);
    }
  };

  const verifyOnboardingReady = async () => {
    const [{ data: authState }, { data: finalProfile }] = await Promise.all([
      api.get("/auth/me"),
      api.get("/profile"),
    ]);
    console.log("ONBOARDING_FINAL_PROFILE_CHECK", {
      auth: authState,
      has_cv_text: Boolean(finalProfile?.cv_text),
      has_cv_filename: Boolean(finalProfile?.cv_filename),
      has_original_cv: Boolean(finalProfile?.cv_original_b64),
      target_role: finalProfile?.target_role,
      target_roles: finalProfile?.target_roles,
    });
    setHasProfile(Boolean(authState?.has_profile));
    setHasPreferences(Boolean(authState?.has_preferences));
    return {
      hasProfile: Boolean(authState?.has_profile),
      hasPreferences: Boolean(authState?.has_preferences),
    };
  };
  ...
  const handlePrefs = async () => {
    ...
    try {
      ...
      const { data } = await api.put("/profile/preferences", {
      ...
      console.log("ONBOARDING_SAVE_PREFERENCES_RESPONSE", data);
      const ready = await verifyOnboardingReady();
      if (!ready.hasProfile) {
        toast.error("Upload your CV before opening jobs");
        setStepIndex(STEP_ORDER.indexOf("upload"));
        return;
      }
      if (!ready.hasPreferences) {
        toast.error("Choose a target role before opening jobs");
        return;
      }
