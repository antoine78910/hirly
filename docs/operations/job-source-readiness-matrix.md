...
| Choisir le Service Public | TS fixture adapter on the generic data.gouv boundary | Evidence recorded as non-production and legal-review-required | Fixture/dataset qualification only | No | None | Confirm current resource, identifiers, expiry/apply route and Open Licence attribution in an approved policy record | Dataset resource/version plus source switch |
| BPCE | TS fixture adapter on the generic data.gouv boundary | Evidence recorded as non-production and legal-review-required | Fixture/dataset qualification only | No | None | Confirm current resource schema, stable IDs, removal semantics and direct apply URLs | Dataset resource/version plus source switch |
| Qualified data.gouv feeds | Generic qualification and fixture/snapshot boundary | Per-resource approval required | Per-resource fixture qualification only | No | None | Freshness, licence, employer attribution, stable IDs, apply route and cohort value | Individual dataset resource/version |
