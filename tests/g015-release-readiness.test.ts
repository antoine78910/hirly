...
describe("G015 release verification contract", () => {
  ...
  test("builds deterministic checks with reproducible installs and typed blockers", () => {
    const databaseUrl = `postgresql://release:super-secret@localhost/${"x".repeat(50)}_test`;
    const verificationId = "20260720120000-123-deadbeef";
    ...
    for (const [index, value] of plannedUrls.entries()) {
  ...
  test("cleans only databases successfully created by the current provisioning invocation", () => {
    const plan = buildReleaseVerificationPlan({
      profile: "full",
      databaseUrl: "postgresql://release:super-secret@localhost/hirly_release_test",
      allowDisposableDatabase: true,
      verificationId: "20260720120000-123-deadbeef",
    });
    const plannedUrls = Object.values(plan.databaseUrls ?? {});
    const [firstUrl, secondUrl] = plannedUrls;
    const firstName = decodeURIComponent(new URL(firstUrl).pathname.slice(1));
    ...
    expect(drops[0]).not.toContain(secondName);
  });
