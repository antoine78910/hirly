...
const migration = readFileSync(
  new URL("../backend/db/migrations/20260720002000_sprout_source_ingestion.sql", import.meta.url),
  "utf8",
);
...
const canaryMigration = readFileSync(
  new URL("../backend/db/migrations/20260720002100_sprout_canary_gate.sql", import.meta.url),
  "utf8",
);
const canaryRollback = readFileSync(
  new URL("../backend/db/migrations/20260720002100_sprout_canary_gate.down.sql", import.meta.url),
  "utf8",
);
...
describe("Sprout source persistence boundary", () => {
  ...
  test("fences the atomic writer by task lease, ownership, policy, and checkpoint", () => {
    ...
    expect(migration).toContain("jsonb_array_length(p_entries) NOT BETWEEN 0 AND 500");
  });
  ...
  test("exposes runnable metadata without persisting credential material", () => {
    ...
    expect(migration).toContain("worker_private.get_sprout_source_runtime");
    expect(migration).toContain("worker_private.career_source_runnable(source.id, 'FR', p_mode)");
    expect(migration).toContain(
      "(source.checkpoint->>'pageSize')::integer = source.approved_page_size",
    );
    expect(migration).toContain("'^secret://[a-z0-9][a-z0-9/_-]{2,127}$'");
    expect(rollback).toContain("DROP FUNCTION IF EXISTS worker_private.get_sprout_source_runtime");
  });
  ...
  test("preserves immutable raw evidence, occurrence identity, and collision evidence", () => {
    ...
    expect(rollback).toContain("refusing to roll back activated or evidence-bearing Sprout source");
  });

  test("reuses one canonical group across duplicate upserts and strong matches", () => {
    expect(migration).toContain("SELECT canonical_group_id INTO v_group_id");
    expect(migration).toContain("IF v_group_id IS NULL THEN");
    expect(migration).toContain("ELSIF v_match_group_id IS NULL THEN");
    expect(migration).toContain("preferred_job_id, merge_confidence, merge_reason");
    expect(migration).toContain("v_group_id, v_match_job_id, 'source_identity', 1");
    expect(migration).toContain("WHERE job_id = v_match_job_id AND canonical_group_id IS NULL");
    expect(migration).toContain("ON CONFLICT (source_id, external_id) DO UPDATE");
  ...
  test("contains no live credential or authorization header material", () => {
    for (const body of [migration, rollback, canaryMigration, canaryRollback, databaseRepository]) {
  ...
  test("adds a default-off DB-backed one-page canary gate", () => {
    expect(canaryMigration).toContain("ADD COLUMN canary_enabled boolean NOT NULL DEFAULT false");
    ...
    expect(canaryMigration).toContain("p_mode IN ('canary', 'incremental', 'backfill')");
    ...
    expect(canaryMigration).toContain("source.canary_evidence->>'pagesCommitted' = '0'");
    expect(canaryMigration).toContain("canary_evidence = CASE");
    expect(canaryMigration).toContain("canary_evidence, '{pagesCommitted}', '1'::jsonb, true");
...
describe("Sprout source commit timestamps", () => {
  test("uses one commit timestamp for ordered attempt and success fields", () => {
    expect(timestampOrderingMigration).toContain("v_source_commit_at := clock_timestamp();");
    expect(timestampOrderingMigration).toContain("last_attempt_at = v_source_commit_at");
    expect(timestampOrderingMigration).toContain("last_success_at = v_source_commit_at");
    expect(timestampOrderingMigration).not.toContain("last_success_at = clock_timestamp()");
  });
});
...
describe("Sprout ingestion error ledger", () => {
  test("retains diagnostic evidence through a lease-fenced, private writer", () => {
    ...
    expect(errorLedgerMigration).toContain(
