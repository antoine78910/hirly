...
export function disposableDatabaseUrlsFromEnvironment(environment = process.env) {
  ...
  for (const url of urls) {
    ...
    const generated = GENERATED_DATABASE_NAME_RE.exec(name);
    if (!generated) {
      throw new Error(`database ${name} was not generated by this verifier run`);
    }
    names.push(name);
    runMarkers.push(generated[1]);
    suites.push(generated[2]);
  }
  if (new Set(names).size !== DATABASE_ENV_NAMES.length) {
    throw new Error("PostgreSQL release suites require eight distinct database names");
  }
  if (suites.some((suite, index) => suite !== GENERATED_DATABASE_SUITE_NAMES[index])) {
    throw new Error("PostgreSQL database names must match their release suites");
  }
  if (new Set(runMarkers).size !== 1) {
    throw new Error("PostgreSQL release suites must share the same verification run");
  }
  return urls;
}
