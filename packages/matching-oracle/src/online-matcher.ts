...
function locationMatches(profile: CandidateSearchProfile, job: JobSearchDocument): boolean {
  ...
  if (
    profile.originLatitude === null ||
    profile.originLongitude === null ||
...
function candidateMatches(profile: CandidateSearchProfile, job: JobSearchDocument): boolean {
  ...
  if (profile.roleFamilyIds.length > 0 && overlap(profile.roleFamilyIds, job.roleFamilyIds) === 0)
    return false;
  if (profile.romeCodes.length > 0 && overlap(profile.romeCodes, job.romeCodes) === 0) return false;
  if (profile.contractTypes.length > 0 && overlap(profile.contractTypes, job.contractTypes) === 0)
    return false;
  ...
  if (
    profile.seniorityMin !== null &&
    job.seniorityMax !== null &&
    job.seniorityMax < profile.seniorityMin
  )
    return false;
  if (
    profile.seniorityMax !== null &&
    job.seniorityMin !== null &&
    job.seniorityMin > profile.seniorityMax
  )
    return false;
  return true;
}
...
function score(profile: CandidateSearchProfile, job: JobSearchDocument, now: Date) {
  ...
  const contract =
    profile.contractTypes.length === 0 || overlap(profile.contractTypes, job.contractTypes) > 0;
  ...
  const explanationCodes = [
    ...
    ...(`${job.fulfillmentRoute}_route` === "auto_route"
      ? ["auto_route" as const]
      : `${job.fulfillmentRoute}_route` === "assisted_route"
        ? ["assisted_route" as const]
...
function emptyResponse(
  ...
  emptyReason: OnlineMatchResponse["emptyReason"],
  counts = { coarse: 0, eligible: 0, hidden: 0 },
): OnlineMatchResponse {
...
export function matchOnline(
  requestInput: OnlineMatchRequest,
  snapshot: OnlineMatcherSnapshot,
): OnlineMatchResponse {
  ...
  if (profile.candidateId !== request.candidateId)
    throw new Error("ONLINE_MATCH_REFUSED: candidate scope mismatch");
  ...
  if (
    actions.some(
      (action) =>
        action.candidateId !== request.candidateId ||
        decimal(action.version) > decimal(request.actionWatermark),
    )
  ) {
    return emptyResponse(request, "PROJECTION_LAG");
  }
  ...
  for (const input of snapshot.jobs) {
    ...
    if (!current || decimal(job.jobVersion) > decimal(current.jobVersion))
      latestJobs.set(job.canonicalGroupId, job);
  }
  const inventory = [...latestJobs.values()];
  const freshInventory = inventory.filter(
    (job) => ageDays(job.publishedAt, now) <= profile.freshnessWindowDays,
  );
  if (freshInventory.length === 0)
    return emptyResponse(request, "NO_FRESH_INVENTORY", {
      coarse: 0,
      eligible: 0,
      hidden: inventory.length,
    });
  const policyVisible = freshInventory.filter(policyEligible);
  if (policyVisible.length === 0)
    return emptyResponse(request, "ALL_POLICY_HIDDEN", {
      coarse: 0,
      eligible: 0,
      hidden: inventory.length,
    });
  ...
  if (eligible.length === 0)
    return emptyResponse(request, "NO_ELIGIBLE_INVENTORY", {
      coarse: 0,
      eligible: 0,
      hidden: inventory.length,
    });
  ...
  if (matching.length === 0)
    return emptyResponse(request, "NO_MATCHING_INVENTORY", {
      coarse: 0,
      eligible: 0,
      hidden: inventory.length,
    });
  ...
  if (visible.length === 0)
    return emptyResponse(request, "ALL_ACTIONED", {
      coarse: matching.length,
      eligible: 0,
      hidden: inventory.length,
    });
  ...
  const results = coarse
    .map((job) => ({ job, ...score(profile, job, now) }))
    .sort(
      (a, b) =>
