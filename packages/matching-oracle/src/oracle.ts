...
function requireValidConfig(config: MatcherConfig): void {
  ...
  if (!Number.isSafeInteger(config.resultLimit) || config.resultLimit < 1 || config.resultLimit > config.coarseLimit) {
...
export class MatchingOracle {
  ...
  constructor(documents: readonly JobSearchDocument[]) {
    for (const document of [...documents].sort((a, b) => a.canonicalGroupId.localeCompare(b.canonicalGroupId))) {
      if (this.documents.has(document.canonicalGroupId)) {
        throw new Error(`MATCHING_ORACLE_REFUSED: duplicate canonical group ${document.canonicalGroupId}`);
      }
      if (!Number.isFinite(Date.parse(document.publishedAt))) {
        throw new Error(`MATCHING_ORACLE_REFUSED: invalid publishedAt for ${document.canonicalGroupId}`);
      }
      if (!Number.isFinite(document.qualityScore) || document.qualityScore < 0 || document.qualityScore > 100) {
        throw new Error(`MATCHING_ORACLE_REFUSED: invalid qualityScore for ${document.canonicalGroupId}`);
      }
  ...
  match(
    ...
    const roleCandidates = [...coarseIds]
      .map((groupId) => this.documents.get(groupId)!);
