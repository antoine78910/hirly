...
import type { CandidateSourceSnapshot } from "./candidate-types";
...
function identifierTokens(value: unknown, maximum: number): string[] {
  return normalizedTokens(value, maximum).map((item) => item.replace(/ /g, "-"));
}
...
function radiusValue(value: unknown): number | null {
  ...
  const match = text(value)?.match(/^(\d+(?:\.\d+)?)\s*km$/i);
...
export function normalizeCandidateProfile(input: {
  ...
  if (!input.snapshot.profile || consentPaused(user)) {
