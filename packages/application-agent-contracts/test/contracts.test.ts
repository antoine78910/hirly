...
describe('application-agent contracts',()=>{
 ...
 test('draft claims retain only an evidence identifier, never classified statement content',()=>{const evidence=fixtureEvidenceItems[0]!; const claim={id:'claim_fixture-a',evidenceId:evidence.id,supportStatus:'supported' as const,verifierReasonCodes:[],confidence:1}; expect(ApplicationClaimSchema.safeParse(claim).success).toBe(true); expect(ApplicationClaimSchema.safeParse({...claim,evidenceStatement:evidence.atomicSupportedStatement}).success).toBe(false); expect(ApplicationClaimSchema.safeParse({...claim,evidenceRef:evidence.sourceArtifactRef}).success).toBe(false);});
});
