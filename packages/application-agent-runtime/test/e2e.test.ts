...
test("fails closed before submit handler, then submits once with exact approval and rejects replay", async () => {
  ...
  const registry = createApplicationAgentOperationRegistry({
    ...
    adapters: fixtureAdapterRegistry(simulator),
  ...
  const frozen: UnsafeValue = await registry.execute(
    ...
    "1.0.0",
    {
      ...
      targetOrigin: job.origin,
      adapterKey: "fixture-ats",
      adapterVersion: "1.0.0",
    },
    ctx,
  );
  const input = {
    planId: frozen.data.id,
    planDigest: frozen.data.planDigest,
    targetOrigin: frozen.data.targetOrigin,
    adapterKey: frozen.data.adapterKey,
  ...
  const receipt = {
    id: "approval_fixture-a",
    subject: { userId: "candidate-a", tenantId: "tenant-a" },
    operation: { key: "hirlyApplication.submit", version: "1.0.0" },
    inputDigest: sha256Hasher.digest(input),
    effects: ["write", "external-side-effect"] as const,
    issuedAt: "2025-12-31T23:00:00.000Z",
    expiresAt: "2026-01-01T01:00:00.000Z",
    nonce: "nonce_fixture-a",
    issuer: "fixture-review",
    evidenceRef: reviewRef,
  };
  approvalPort = createApprovalPort({
  ...
  const registry = createApplicationAgentOperationRegistry({
    ...
    model: {
      async createDraft(args) {
        modelCalls++;
        return { ...(await fixtureModel.createDraft(args)), id: "draft_fixture-port" };
      },
    },
    verifier: {
      async verify() {
    ...
    adapters: fixtureAdapterRegistry(controlledAtsSimulator()),
    idempotency: memoryIdempotencyStore(),
    clock,
    ids: incrementalIds(),
    hasher: sha256Hasher,
...
test("verification resolves persisted candidate evidence after runtime reconstruction", async () => {
  ...
  const dependencies = (verifier: UnsafeValue) => ({
    ...
    jobs: memoryStore([job]),
    drafts,
    plans: memoryStore<ApplicationSubmissionPlan>(),
    receipts: memoryReceiptStore(),
    reader: fixtureJobReader(job),
    model: fixtureModel,
    verifier,
...
test("operation registry rejects a production composition request before binding handlers", () => {
  ...
  expect(() =>
    createApplicationAgentOperationRegistry({ ...deps, compositionMode: "production" }),
  ).toThrow("PRODUCTION_COMPOSITION_UNAVAILABLE");
});

test("approval port rejects wrong subject, canonical input, and review evidence before a handler can run", async () => {
  const input = {
    planId: "submission_plan_fixture-a",
    planDigest: sha256Hasher.digest({ plan: 1 }),
    targetOrigin: "https://jobs.fixture.example",
    adapterKey: "fixture-ats",
    adapterVersion: "1.0.0",
    idempotencyKey: "idem_fixture_12345678",
  };
  const review = {
    ref: "review:fixture-approval",
    status: "approved" as const,
    subject: { userId: "candidate-a" },
    planDigest: input.planDigest,
    targetOrigin: input.targetOrigin,
    adapterVersion: input.adapterVersion,
    issuedAt: "2025-12-31T23:00:00.000Z",
  ...
  const port = createApprovalPort({
    reviews: memoryReviewStore([review]),
    nonces: memoryNonceStore(),
    clock,
    hasher: sha256Hasher,
  });
  ...
  const request: UnsafeValue = {
    ...
    input,
    requiredEffects: receipt.effects,
  };
  ...
  expect(
    (
      await port.authorize({
        ...request,
        input: { ...input, targetOrigin: "https://wrong.example" },
      })
    ).effect,
  ).toBe("deny");
  expect(
  ...
  expect(
    (
      await createApprovalPort({
        reviews: memoryReviewStore([{ ...review, expiresAt: "2025-12-31T23:59:59.000Z" }]),
        nonces: memoryNonceStore(),
        clock,
        hasher: sha256Hasher,
      }).authorize(request)
...
test("approval digest is characterized against the installed runtime-core serializer without declaring it as a dependency", async () => {
  const install = readdirSync(join(process.cwd(), "../../node_modules/.bun")).find((entry) =>
  ...
  const input = {
    z: [{ beta: true, alpha: new Date("2026-01-01T00:00:00.000Z") }],
    a: undefined,
    b: { z: 1, a: "x" },
  };
...
test("runtime event publisher rejects undeclared and malformed events before publishing", async () => {
  ...
  await expect(publisher.publish("hirlyApplication.unknown", "1.0.0", {})).rejects.toThrow(
    "UNDECLARED_APPLICATION_AGENT_EVENT",
  );
  await expect(
    publisher.publish("hirlyApplication.prepared", "1.0.0", { eventId: "event_fixture-a" }),
  ).rejects.toThrow("MALFORMED_APPLICATION_AGENT_EVENT");
  await publisher.publish("hirlyApplication.prepared", "1.0.0", {
    eventId: "event_fixture-a",
...
test("submit reloads persisted plan sources and blocks changed job snapshots before mutation", async () => {
  ...
  const drafts = memoryStore<ApplicationDraft>();
  const plans = memoryStore<ApplicationSubmissionPlan>();
  const simulator = controlledAtsSimulator();
  let approvalPort: ReturnType<typeof createApprovalPort> | undefined;
  const registry = createApplicationAgentOperationRegistry({
    evidence: memoryEvidenceStore(fixtureEvidenceSnapshot, fixtureEvidenceItems),
...
test("safe logger rejects arbitrary candidate prose and persists only constrained fields", () => {
  ...
  ).toThrow("UNSAFE_APPLICATION_AGENT_LOG_RECORD");
  logger.info("application_agent_operation", {
    subjectRef: "candidate:fixture-a",
    entityRef: "plan:fixture-a",
    reasonCodes: ["STALE_SUBMISSION_PLAN"],
  });
  expect(logger.records).toEqual([
    {
      level: "info",
      event: "application_agent_operation",
      fields: {
        subjectRef: "candidate:fixture-a",
        entityRef: "plan:fixture-a",
        reasonCodes: ["STALE_SUBMISSION_PLAN"],
...
test("candidate MCP allowlist excludes server-only outcome observation", () => {
  const job = { ...fixtureJobSnapshot, questions: [] };
  const registry = createCandidateMcpOperationRegistry({
    evidence: memoryEvidenceStore(fixtureEvidenceSnapshot, fixtureEvidenceItems),
    jobs: memoryStore(),
    drafts: memoryStore(),
    plans: memoryStore(),
  ...
  expect(registry.list().map((spec) => spec.meta.key)).toEqual([
    "hirlyJob.analyze",
    "hirlyApplication.prepare",
    "hirlyApplication.verify",
    "hirlyApplication.freeze",
...
test("MCP candidate tools use host identity and host-only approval receipts", async () => {
  ...
  const deps: RuntimeDependencies = {
    evidence: memoryEvidenceStore(fixtureEvidenceSnapshot, fixtureEvidenceItems),
    jobs: memoryStore<JobSnapshot>(),
    drafts: memoryStore<ApplicationDraft>(),
    plans: memoryStore<ApplicationSubmissionPlan>(),
    receipts: memoryReceiptStore(),
    reader: fixtureJobReader(job),
    model: fixtureModel,
  ...
  const prepared = await call("hirlyApplication.prepare", {
    ...
    jobSnapshotId: contractData(analyzed).id,
  });
  const frozen = await call("hirlyApplication.freeze", {
    draftId: contractData(prepared).id,
    targetOrigin: job.origin,
    adapterKey: "fixture-ats",
    adapterVersion: "1.0.0",
  });
  const input = {
    planId: contractData(frozen).id,
    planDigest: contractData(frozen).planDigest,
    targetOrigin: contractData(frozen).targetOrigin,
    adapterKey: contractData(frozen).adapterKey,
    adapterVersion: contractData(frozen).adapterVersion,
    idempotencyKey: contractData(frozen).idempotencyKey,
  };
...
test("public package entry exposes composition but no direct handler module", async () => {
  const api = await import("../src/index");
  expect(api).toHaveProperty("createApplicationAgentOperationRegistry");
  expect(api).not.toHaveProperty("submitApplicationHandler");
  expect(Object.keys(api).every((key) => !/handler/i.test(key))).toBe(true);
});
