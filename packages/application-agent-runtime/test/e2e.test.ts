...
test('operation registry rejects a production composition request before binding handlers', () => {
  ...
  expect(() => createApplicationAgentOperationRegistry({ ...deps, compositionMode: 'production' })).toThrow('PRODUCTION_COMPOSITION_UNAVAILABLE');
});
...
test('safe logger rejects arbitrary candidate prose and persists only constrained fields', () => {
  ...
  expect(() => (logger.info as any)('application_agent_operation', { candidateName: 'Ada Example', rawCv: 'private CV' })).toThrow('UNSAFE_APPLICATION_AGENT_LOG_RECORD');
  logger.info('application_agent_operation', { subjectRef: 'candidate:fixture-a', entityRef: 'plan:fixture-a', reasonCodes: ['STALE_SUBMISSION_PLAN'] });
  expect(logger.records).toEqual([{ level: 'info', event: 'application_agent_operation', fields: { subjectRef: 'candidate:fixture-a', entityRef: 'plan:fixture-a', reasonCodes: ['STALE_SUBMISSION_PLAN'] } }]);
});
