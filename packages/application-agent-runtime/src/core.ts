...
export const canonicalJson = (value: unknown): string => {
  ...
  if (typeof value === 'number') { if (!Number.isFinite(value)) throw new TypeError('Approval input is not JSON.'); return JSON.stringify(value); }
  if (Array.isArray(value)) return `[${value.map(canonicalJson).join(',')}]`;
  if (value instanceof Date) return JSON.stringify(value.toISOString());
  if (typeof value === 'object') return `{${Object.entries(value as Record<string, unknown>).filter(([, item]) => item !== undefined).sort(([a], [b]) => a.localeCompare(b)).map(([key, item]) => `${JSON.stringify(key)}:${canonicalJson(item)}`).join(',')}}`;
...
export const incrementalIds = (): IdGenerator => { let i = 0; return { next: (prefix) => `${prefix}_fixture-${++i}` }; };
export const memoryNonceStore = (): ApprovalNonceStore => { const used = new Set<string>(); return { async consume(nonce) { if (used.has(nonce)) return false; used.add(nonce); return true; } }; };
export const memoryIdempotencyStore = (): IdempotencyStore => { const used = new Set<string>(); return { async claim(key) { if (used.has(key)) return 'duplicate'; used.add(key); return 'claimed'; } }; };
...
export const assertFixtureOnlyMode = (mode: 'fixture' | 'production') => {
  if (mode !== 'fixture') throw new Error('PRODUCTION_COMPOSITION_UNAVAILABLE: durable stores and a separately approved production adapter are required');
};
