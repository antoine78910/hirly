...
import type { ApplicationDraft, ApplicationSubmissionPlan, ApplicationSubmissionReceipt } from '@hirly/application-agent-contracts';
import type {
  ApplicationDraftStore, ApplicationModelGateway, AtsCapabilityRegistry,
  CandidateEvidenceStore, ClaimSupportVerifier, Clock, Hasher, IdGenerator,
  IdempotencyStore, JobSnapshotStore, JobSourceReader, SafeLogger,
...
const safeEvent = (deps: RuntimeDependencies, values: Record<string, unknown>) => ({
  eventId: deps.ids.next('event'), subjectRef: 'candidate:fixture-a',
  occurredAt: deps.clock.now().toISOString(), ...values,
});
...
export const createApplicationAgentOperationRegistry = (deps: RuntimeDependencies) => {
  ...
  bind(analyzeJobOperation, async (input: any) => {
    try { const job = await deps.reader.read(input); await deps.jobs.put(job); return contractOk({ ok: true, data: job }); }
    catch { return failure(input.jobUrl ? 'JOB_EXTRACTION_FAILED' : 'INVALID_JOB_URL'); }
  });
  ...
  bind(prepareApplicationOperation, async (input: any) => {
    ...
    const hasUnboundClaim = generated.claims.some((claim) => !evidence.some((item) => item.id === claim.evidenceId));
    ...
    await events.publish(eventName, '1.0.0', safeEvent(deps, { draftId: draft.id, jobSnapshotFingerprint: draft.jobSnapshotFingerprint }));
  ...
  bind(freezeApplicationOperation, async (input: any) => {
    ...
    if (verification.blockedReasonCodes.includes('MISSING_CANDIDATE_INPUT')) return failure('MISSING_CANDIDATE_INPUT');
    if (verification.blockedReasonCodes.length) return failure('UNSUPPORTED_APPLICATION_CLAIM');
    const base = { id: deps.ids.next('submission_plan'), version: 1, draftId: draft.id, candidateEvidenceSnapshotId: draft.candidateEvidenceSnapshotId, jobSnapshotId: draft.jobSnapshotId, candidateSnapshotFingerprint: draft.candidateSnapshotFingerprint, jobSnapshotFingerprint: draft.jobSnapshotFingerprint, draftFingerprint: deps.hasher.digest(draft), secureFormPayloadRef: 'vault:form-payload', formPayloadFingerprint: deps.hasher.digest({ draft: draft.id }), targetOrigin: input.targetOrigin, adapterKey: input.adapterKey, adapterVersion: input.adapterVersion, allowedActionIds: ['submit'], idempotencyKey: `idem_${draft.id.replace('draft_', '')}_12345678`, expiresAt: new Date(deps.clock.now().getTime() + 3_600_000).toISOString() };
    const plan: ApplicationSubmissionPlan = { ...base, planDigest: deps.hasher.digest(base) };
    await deps.plans.put(plan);
    await events.publish('hirlyApplication.planFrozen', '1.0.0', safeEvent(deps, { planId: plan.id }));
  ...
  bind(submitApplicationOperation, async (input: any, ctx: any) => {
    ...
    if (plan.adapterKey !== input.adapterKey || plan.adapterVersion !== input.adapterVersion) return failure('ADAPTER_CAPABILITY_MISMATCH');
    if (plan.idempotencyKey !== input.idempotencyKey || plan.expiresAt <= deps.clock.now().toISOString()) return failure('STALE_SUBMISSION_PLAN');
    const base = { ...plan }; delete (base as any).planDigest;
    ...
    if (!draft || !snapshot || !job || draft.candidateEvidenceSnapshotId !== snapshot.id || draft.jobSnapshotId !== job.id || draft.candidateSnapshotFingerprint !== snapshot.snapshotFingerprint || draft.jobSnapshotFingerprint !== job.sourceFingerprint || snapshot.snapshotFingerprint !== plan.candidateSnapshotFingerprint || job.sourceFingerprint !== plan.jobSnapshotFingerprint || deps.hasher.digest(draft) !== plan.draftFingerprint) return failure('STALE_SUBMISSION_PLAN');
    const evidence = await deps.evidence.getItems(snapshot.evidenceItemIds);
    if (evidence.length !== snapshot.evidenceItemIds.length || !snapshot.evidenceItemIds.every((id) => evidence.some((item) => item.id === id))) return failure('STALE_SUBMISSION_PLAN');
    const verification = await deps.verifier.verify(draft, evidence);
    if (verification.blockedReasonCodes.includes('MISSING_CANDIDATE_INPUT')) return failure('MISSING_CANDIDATE_INPUT');
    if (verification.blockedReasonCodes.length) return failure('UNSUPPORTED_APPLICATION_CLAIM');
    const adapter = deps.adapters.get(plan.adapterKey, plan.adapterVersion);
    if (!adapter || !adapter.capabilities.submit || adapter.capabilities.captchaSupport !== false || adapter.capabilities.submissionApprovalRequired !== true) return failure('ADAPTER_CAPABILITY_MISMATCH');
    if (await deps.idempotency.claim(plan.idempotencyKey) !== 'claimed') return failure('IDEMPOTENCY_CONFLICT');
    await events.publish('hirlyApplication.submissionAttempted', '1.0.0', safeEvent(deps, { planId: plan.id }));
    try {
      ...
      if (!readBack.confirmed || !readBack.evidenceRef) {
        await events.publish('hirlyApplication.submissionFailed', '1.0.0', safeEvent(deps, { planId: plan.id, reasonCodes: ['SUBMISSION_NOT_CONFIRMED'] }));
        return failure('SUBMISSION_NOT_CONFIRMED');
      }
      const receipt: ApplicationSubmissionReceipt = { attemptId: deps.ids.next('attempt'), planId: plan.id, planDigest: plan.planDigest, approvalReceiptId: ctx.approvalReceipt.id, idempotencyKey: plan.idempotencyKey, targetOrigin: plan.targetOrigin, adapterKey: plan.adapterKey, adapterVersion: plan.adapterVersion, status: 'submitted', providerApplicationId: submitted.providerApplicationId, safeEvidenceRefs: [readBack.evidenceRef], observedAt: deps.clock.now().toISOString() };
      await deps.receipts.put(receipt);
      await events.publish('hirlyApplication.submitted', '1.0.0', safeEvent(deps, { planId: plan.id, attemptId: receipt.attemptId, safeEvidenceRefs: receipt.safeEvidenceRefs }));
      return contractOk({ ok: true, data: receipt });
    } catch (error) {
      const code = String(error).includes('RATE_LIMIT') ? 'PROVIDER_RATE_LIMITED' : 'INTERNAL_ERROR';
      await events.publish('hirlyApplication.submissionFailed', '1.0.0', safeEvent(deps, { planId: plan.id, reasonCodes: [code] }));
      return failure(code);
    }
  });
