...
export type GuardedEventPublisher = { publish(key: string, version: string, payload: unknown): Promise<void> };

/** Resolves only declared application-agent events and validates payloads before they leave the runtime. */
export const createGuardedEventPublisher = (outbox: AuditOutboxPublisher): GuardedEventPublisher => {
  ...
  return {
    async publish(key, version, payload) {
      ...
      await outbox.publish({ key: event.meta.key, version: event.meta.version, payload: parsed.data });
    },
  };
};
