...
const usage = "usage: ats-inventory-shadow <run|seal> --provider <greenhouse|recruitee|nicoka> --tenant <exact-tenant> --country FR --policy <path> --output <path> --evidence-root <path> --live | seal --run <path> --run <path> --output <path> --evidence-root <path>";
...
export function parseAtsInventoryShadowArgs(args: string[]): AtsInventoryShadowCliCommand {
  ...
  if (type === "run") {
    ...
    return { type, provider: provider.data, tenantId, countryCode: "FR", policyPath: one("--policy")!, outputPath: one("--output")!, evidenceRootPath: one("--evidence-root")! };
  }
  ...
  return { type, runPaths: [runs[0]!, runs[1]!], outputPath: one("--output")!, evidenceRootPath: one("--evidence-root")! };
}
...
async function resolveEvidenceRoot(path: string): Promise<string> {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) throw new Error("evidence root must be a real directory, not a symlink");
...
async function ensureRealDirectory(path: string, root: string): Promise<void> {
  ...
  if (relativePath === ".." || relativePath.startsWith(`..${sep}`)) throw new Error("artifact path must be contained by the evidence root");
  await mkdir(path, { recursive: true });
  let current = root;
  for (const segment of relativePath ? relativePath.split(sep) : []) {
    current = resolve(current, segment);
    const stat = await lstat(current);
    if (!stat.isDirectory() || stat.isSymbolicLink()) throw new Error("artifact path must not traverse a symlink");
  }
}
...
if (import.meta.main) {
  ...
  console.log(JSON.stringify({ runId: "runId" in result ? result.runId : undefined, output: command.outputPath, canonicalWritesEnabled: false }));
}
