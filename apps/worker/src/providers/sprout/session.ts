...
export class SproutSessionCipher {
  ...
  decrypt(encoded: string): SproutSession {
    ...
    try {
      ...
      const decoded = JSON.parse(Buffer.concat([decipher.update(ciphertext), decipher.final()]).toString("utf8")) as Partial<SproutSession>;
      if (typeof decoded.accessToken !== "string" || !decoded.accessToken.trim() || typeof decoded.refreshToken !== "string" || !decoded.refreshToken.trim()) {
