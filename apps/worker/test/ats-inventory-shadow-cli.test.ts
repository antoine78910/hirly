...
describe("ATS inventory shadow CLI", () => {
  ...
  test("permits and seals Greenhouse evidence only after total reconciliation", async () => {
    ...
    try {
      ...
      await expect(runAtsInventoryShadowCli(runCommand(root, firstPath), {
        now: () => now,
        makeRunId: () => "run-one",
        fetch: async () => greenhouseResponse("1"), evidenceHmacKey,
  ...
  test("does not write evidence when Greenhouse total reconciliation fails", async () => {
    ...
    try {
      ...
      await expect(runAtsInventoryShadowCli(runCommand(root, output), {
        now: () => now,
        makeRunId: () => "run-one",
        fetch: async () => Response.json({ jobs: [], meta: { total: 1 } }), evidenceHmacKey,
  ...
  test("refuses unsigned, forged, and tampered hand-authored run JSON", async () => {
    ...
    try {
      const baseRun = {
        schemaVersion: "job-supply-shadow-run.v1", runId: "run-one", provider: "greenhouse",
        tenantId: "vaulttec", countryCode: "FR", policyDigest: "a".repeat(64),
        complete: true, canonicalWritesEnabled: false, capturedAt: "2026-07-21T00:00:00.000Z",
      ...
      const commandFor = (firstPath: string, secondPath: string) => parseAtsInventoryShadowArgs([
        "seal", "--run", firstPath, "--run", secondPath, "--output", scorecardPath, "--evidence-root", root,
      ]);
      for (const [name, first] of [
        ["unsigned", baseRun],
        ["forged", { ...baseRun, signature: "0".repeat(64) }],
        ["tampered", { ...signedRun(baseRun), jobs: [{ externalId: "vaulttec:forged", fingerprint: "b".repeat(64) }] }],
      ] as const) {
        ...
        await expect(runAtsInventoryShadowCli(commandFor(firstPath, secondPath), { evidenceHmacKey }))
  ...
  test("requires a secure shadow evidence HMAC key", async () => {
    ...
    try {
      await writeFile(join(root, "policy.json"), JSON.stringify(policy()));
      await expect(runAtsInventoryShadowCli(runCommand(root, join(root, "runs/first.json")), { evidenceHmacKey: "too-short" }))
