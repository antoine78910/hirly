...
const summary = [
  ...
  "- Additive worker run/partition accounting migration and alert view.",
  "- Deterministic TypeScript audit command, fixtures, and machine-readable evidence.",
  "",
  "## Tests and commands run",
  "",
  "- `.venv/bin/python -m pytest -q backend/tests/test_france_travail_provider.py backend/tests/test_job_cache_maintenance.py backend/tests/test_feed_db_first.py backend/tests/test_server_startup_tasks.py`",
  ...
  "- The existing Python scheduler remains authoritative; the additive TypeScript ledger does not transfer ownership.",
  "- Feed offset/keyset performance and exact ordered IDs require representative DB evidence.",
  "",
  "## Rollback procedure",
  "",
  "1. Revert the Python paging/maintenance/task-retention commits if operational regression is observed.",
