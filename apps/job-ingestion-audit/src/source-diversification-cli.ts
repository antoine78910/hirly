...
import {
  ...
  type SourceDiversificationGateInput,
...
for (let index = 0; index < Bun.argv.slice(2).length; index += 2) {
  ...
  if (!["--input", "--output"].includes(name ?? "") || !value) {
    throw new Error(
      "usage: source-diversification --input <aggregate.json> --output <report.json>",
    );
  }
...
) as SourceDiversificationGateInput;
const report = buildSourceDiversificationGate(input);
