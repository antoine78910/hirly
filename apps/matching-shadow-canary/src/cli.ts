...
const defaultOutputPath = resolve(
  import.meta.dir,
  "../../../artifacts/candidate-matching/g008-shadow-canary.json",
);
...
export async function runShadowCanaryCli(args = process.argv.slice(2)): Promise<void> {
  ...
  if (visibleLegacyResponses !== 1)
    throw new Error("shadow canary must expose exactly one legacy response");

  const outputPath = resolve(argument(args, "output") ?? defaultOutputPath);
  await mkdir(dirname(outputPath), { recursive: true });
  await writeFile(outputPath, `${JSON.stringify(evidence, null, 2)}\n`);
  process.stdout.write(`${outputPath}\n`);
  if (execute && !evidence.decision.automaticRollback && injectedBreach !== "none")
    process.exitCode = 2;
}
