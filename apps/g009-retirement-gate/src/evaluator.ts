...
export function evaluateG009RetirementGate(
  ...
  const evidenceKind =
    input.evidenceKind === "fixture" || input.evidenceKind === "production"
      ? input.evidenceKind
      : "none";
  ...
  const delta =
    legacyRate === null || v2Rate === null ? null : Math.round((v2Rate - legacyRate) * 100) / 100;
  const lowerBound =
    typeof input.lowerConfidenceBoundDeltaPercentagePoints === "number" &&
    Number.isFinite(input.lowerConfidenceBoundDeltaPercentagePoints)
      ? input.lowerConfidenceBoundDeltaPercentagePoints
      : null;
  const invariantFailures: string[] = [];
  if (hasWholeCountViolation(input.healthyDays))
    invariantFailures.push("healthy_days_must_be_a_whole_count");
  if (hasWholeCountViolation(input.v2Requests))
    invariantFailures.push("v2_requests_must_be_a_whole_count");
  if (
    hasWholeCountViolation(input.activePaidProfilesCovered) ||
    hasWholeCountViolation(input.activePaidProfilesTotal)
  ) {
  ...
  else if (healthyDays < G009_RETIREMENT_THRESHOLDS.healthyDays)
    reasons.push("healthy_days_below_30");
  if (v2Requests === null) reasons.push("v2_requests_missing");
  else if (v2Requests < G009_RETIREMENT_THRESHOLDS.v2Requests)
    reasons.push("v2_requests_below_10000");
  if (paidCoverage === null) reasons.push("active_paid_coverage_missing");
  else if (paidCoverage < G009_RETIREMENT_THRESHOLDS.activePaidCoveragePercent)
    reasons.push("active_paid_coverage_below_80_percent");
  ...
  if (evidenceKind === "production" && evidenceRefs.length === 0)
    reasons.push("production_evidence_refs_missing");
