...
describe("Feed v2 runtime adapters", () => {
  test("keeps runtime routing disabled without database or secret configuration", () => {
    ...
    expect(() => parseFeedV2Config({ FEED_V2_ROUTING_ENABLED: "true" })).toThrow(
      "JOBS_DATABASE_URL",
    );
    expect(
      parseFeedV2Config({
    ...
    expect(() => parseFeedV2Config({ FEED_V2_ROUTING_ENABLED: "maybe" })).toThrow(
  ...
  test("verifies bounded signed assertions and rejects tampering", async () => {
    ...
    const request = new Request("http://feed.test/internal/feed/v2", {
      headers: {
        "x-hirly-feed-assertion": signed.encodedAssertion,
        "x-hirly-feed-signature": signed.signature,
      },
    });
    const verifier = new HmacFeedAssertionVerifier(secret, {
  ...
  test("binds the effective explicit query to both its fingerprint and assertion signature", async () => {
    ...
    expect(
      await verifier.verify(
        new Request("http://feed.test/internal/feed/v2", {
          headers: {
            "x-hirly-feed-assertion": signed.encodedAssertion,
            "x-hirly-feed-signature": signed.signature,
          },
  ...
  test("uses candidate-scoped indexed keyset SQL without any mutation surface", async () => {
    ...
    const repository = new PostgresFeedReadRepository({
      async unsafe(query, parameters) {
        ...
        return [
          {
            profile_version: "7#candidate-profile",
            action_watermark: "9",
            snapshot_version: "inventory-1#candidate-profile",
            query_fingerprint: "candidate-profile",
    ...
    expect(calls).toEqual([
      {
        query: FEED_V2_INDEXED_READ_SQL,
        parameters: [
          "candidate-1",
          0.8,
          "group-0",
    ...
    expect(FEED_V2_INDEXED_READ_SQL).toContain(
    ...
    expect(FEED_V2_INDEXED_READ_SQL).not.toMatch(
      /provider_registry|worker_tasks|projection_reconciliation_tasks/i,
    );
  });
  test("executes signed role, radius, country, work-mode, and contract filters as parameters", async () => {
    ...
    expect(FEED_V2_INDEXED_READ_SQL).toContain(
      "array_append(profile.target_role_labels_normalized, effective_query.role)",
    );
    ...
    expect(FEED_V2_INDEXED_READ_SQL).toContain("LEAST(1.0");
  });
});
...
describe("Feed v2 peak serving query", () => {
  test("scores the complete active inventory before keyset pagination", () => {
    ...
    expect(FEED_V2_INDEXED_READ_SQL).toMatch(
      /FROM recency_candidates AS document[\s\S]*ORDER BY relevance_score DESC, canonical_group_id ASC/,
    );
