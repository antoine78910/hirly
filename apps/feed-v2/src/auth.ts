...
function validAssertion(value: unknown): value is FeedAuthAssertion {
  ...
  return (
    (keys === "candidateId,expiresAt,issuedAt,scopes,subject" ||
    ...
    typeof assertion.candidateId === "string" &&
    assertion.candidateId.length > 0 &&
    assertion.candidateId.length <= 256 &&
    Array.isArray(assertion.scopes) &&
    assertion.scopes.length <= 16 &&
    assertion.scopes.every(
      (scope) => typeof scope === "string" && scope.length > 0 && scope.length <= 64,
    ) &&
    typeof assertion.issuedAt === "string" &&
    Number.isFinite(Date.parse(assertion.issuedAt)) &&
    typeof assertion.expiresAt === "string" &&
...
export function signFeedAssertion(
...
export class HmacFeedAssertionVerifier implements FeedAuthAssertionVerifier {
  constructor(
    ...
    if (secret.length < 32)
      throw new Error("FEED_V2_ASSERTION_SECRET must contain at least 32 characters");
  }
  ...
  async verify(request: Request): Promise<FeedAuthAssertion> {
    ...
    if (!encoded || !providedHex || !/^[a-f0-9]{64}$/i.test(providedHex))
      throw new Error("invalid_assertion_signature");
    ...
    if (issuedAt > now + 30_000 || expiresAt <= now || expiresAt <= issuedAt)
      throw new Error("invalid_assertion_time");
    if (expiresAt - issuedAt > (this.options.maximumLifetimeMs ?? 5 * 60_000))
      throw new Error("assertion_lifetime_exceeded");
    return parsed;
  }
}
