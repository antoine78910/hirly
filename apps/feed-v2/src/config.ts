...
export function parseFeedV2Config(env: Record<string, string | undefined>): FeedV2RuntimeConfig {
  ...
  if (
    env.FEED_V2_ROUTING_ENABLED !== undefined &&
    !["true", "false"].includes(env.FEED_V2_ROUTING_ENABLED)
  ) {
  ...
  if (routingEnabled && !databaseUrl)
    throw new Error("JOBS_DATABASE_URL is required when Feed v2 routing is enabled");
  if (routingEnabled && (!assertionSecret || assertionSecret.length < 32)) {
    throw new Error(
      "FEED_V2_ASSERTION_SECRET must contain at least 32 characters when routing is enabled",
    );
  }
  return {
    routingEnabled,
    ...
    requestTimeoutMs: integer(
      "FEED_V2_REQUEST_TIMEOUT_MS",
      env.FEED_V2_REQUEST_TIMEOUT_MS,
      1_500,
      100,
      10_000,
    ),
