-- TS_NEW: default-off operational Sprout canary and release-evidence gate.
-- No provider/source is activated and no credential material is stored here.
BEGIN;

ALTER TABLE public.career_sources
  ADD COLUMN canary_enabled boolean NOT NULL DEFAULT false,
  ADD COLUMN canary_evidence jsonb NOT NULL DEFAULT '{"status":"pending","evidenceRef":null,"pagesCommitted":0,"identityReadBack":false,"rawSnapshotLinked":false,"occurrenceLinked":false,"checkpointReadBack":false,"singleWriterVerified":false}'::jsonb,
  ADD COLUMN rollback_evidence jsonb NOT NULL DEFAULT '{"status":"pending","evidenceRef":null,"providerKillSwitchVerified":false,"sourceKillSwitchVerified":false,"scheduleDisableVerified":false,"transportDisableVerified":false,"outstandingTasksStopVerified":false,"writerClaimReleaseVerified":false}'::jsonb,
  ADD CONSTRAINT career_sources_canary_evidence_guard CHECK (
    jsonb_typeof(canary_evidence) = 'object'
    AND canary_evidence ?& ARRAY[
      'status', 'evidenceRef', 'pagesCommitted', 'identityReadBack',
      'rawSnapshotLinked', 'occurrenceLinked', 'checkpointReadBack',
      'singleWriterVerified'
    ]
    AND canary_evidence->>'status' IN ('pending', 'failed', 'passed')
    AND canary_evidence->>'pagesCommitted' ~ '^[01]$'
    AND jsonb_typeof(canary_evidence->'identityReadBack') = 'boolean'
    AND jsonb_typeof(canary_evidence->'rawSnapshotLinked') = 'boolean'
    AND jsonb_typeof(canary_evidence->'occurrenceLinked') = 'boolean'
    AND jsonb_typeof(canary_evidence->'checkpointReadBack') = 'boolean'
    AND jsonb_typeof(canary_evidence->'singleWriterVerified') = 'boolean'
  ),
  ADD CONSTRAINT career_sources_rollback_evidence_guard CHECK (
    jsonb_typeof(rollback_evidence) = 'object'
    AND rollback_evidence ?& ARRAY[
      'status', 'evidenceRef', 'providerKillSwitchVerified',
      'sourceKillSwitchVerified', 'scheduleDisableVerified',
      'transportDisableVerified', 'outstandingTasksStopVerified',
      'writerClaimReleaseVerified'
    ]
    AND rollback_evidence->>'status' IN ('pending', 'failed', 'passed')
    AND jsonb_typeof(rollback_evidence->'providerKillSwitchVerified') = 'boolean'
    AND jsonb_typeof(rollback_evidence->'sourceKillSwitchVerified') = 'boolean'
    AND jsonb_typeof(rollback_evidence->'scheduleDisableVerified') = 'boolean'
    AND jsonb_typeof(rollback_evidence->'transportDisableVerified') = 'boolean'
    AND jsonb_typeof(rollback_evidence->'outstandingTasksStopVerified') = 'boolean'
    AND jsonb_typeof(rollback_evidence->'writerClaimReleaseVerified') = 'boolean'
  );
...
AS $$
  SELECT EXISTS (
    ...
    WHERE source.id = p_source_id
      ...
      AND p_mode IN ('canary', 'incremental', 'backfill')
      ...
      END
      AND (
        source.provider <> 'sprout'
        OR p_mode = 'canary'
        OR (
          source.canary_evidence->>'status' = 'passed'
          AND source.canary_evidence->>'evidenceRef' <> ''
          AND (source.canary_evidence->>'pagesCommitted')::integer = 1
          AND (source.canary_evidence->>'identityReadBack')::boolean
          AND (source.canary_evidence->>'rawSnapshotLinked')::boolean
          AND (source.canary_evidence->>'occurrenceLinked')::boolean
          AND (source.canary_evidence->>'checkpointReadBack')::boolean
          AND (source.canary_evidence->>'singleWriterVerified')::boolean
          AND source.rollback_evidence->>'status' = 'passed'
          AND source.rollback_evidence->>'evidenceRef' <> ''
          AND (source.rollback_evidence->>'providerKillSwitchVerified')::boolean
          AND (source.rollback_evidence->>'sourceKillSwitchVerified')::boolean
          AND (source.rollback_evidence->>'scheduleDisableVerified')::boolean
          AND (source.rollback_evidence->>'transportDisableVerified')::boolean
          AND (source.rollback_evidence->>'outstandingTasksStopVerified')::boolean
          AND (source.rollback_evidence->>'writerClaimReleaseVerified')::boolean
        )
      )
      AND NOT coalesce(
...
DROP FUNCTION worker_private.get_sprout_source_runtime(uuid, text);
CREATE FUNCTION worker_private.get_sprout_source_runtime(
  p_source_id uuid,
  p_mode text
)
RETURNS TABLE (
...
AS $$
  ...
  WHERE source.id = p_source_id
    ...
    AND (
      p_mode <> 'canary'
      OR (
        source.checkpoint->>'offset' = '0'
        AND source.canary_evidence->>'status' = 'pending'
        AND source.canary_evidence->>'pagesCommitted' = '0'
      )
    )
$$;
...
BEGIN
  IF upper(p_country_code) <> 'FR'
    OR p_mode NOT IN ('canary', 'incremental', 'backfill')
    OR jsonb_typeof(p_checkpoint_in) <> 'object'
    OR jsonb_typeof(p_checkpoint_out) <> 'object'
    OR (
      p_mode = 'canary'
      AND (
        coalesce(p_checkpoint_in->>'offset', '') <> '0'
        OR coalesce(p_checkpoint_out->>'offset', '') !~ '^[1-9][0-9]*$'
      )
    )
  ...
  WHERE task.id = p_task_id
    ...
    AND (
      p_mode <> 'canary'
      OR (
        source.canary_evidence->>'status' = 'pending'
        AND source.canary_evidence->>'pagesCommitted' = '0'
      )
    )
  ...
  SET checkpoint = p_checkpoint_out,
      canary_evidence = CASE
        WHEN p_mode = 'canary' THEN jsonb_set(
          canary_evidence, '{pagesCommitted}', '1'::jsonb, true
        )
        ELSE canary_evidence
      END,
...
REVOKE ALL ON FUNCTION worker_private.get_sprout_source_runtime(uuid, text) FROM PUBLIC;
