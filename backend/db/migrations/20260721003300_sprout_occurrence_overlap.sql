-- TS_MIGRATION: permit independent Sprout discovery lanes to overlap safely.
-- job_occurrences has one canonical occurrence per job_id; per-lane source evidence lives
-- in raw_job_snapshots, so an existing occurrence must retain its source-bound snapshot.
BEGIN;
