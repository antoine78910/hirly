...
BEGIN
  IF EXISTS (
    SELECT 1 FROM public.career_sources
    WHERE canary_enabled
      OR canary_evidence <> '{"status":"pending","evidenceRef":null,"pagesCommitted":0,"identityReadBack":false,"rawSnapshotLinked":false,"occurrenceLinked":false,"checkpointReadBack":false,"singleWriterVerified":false}'::jsonb
      OR rollback_evidence <> '{"status":"pending","evidenceRef":null,"providerKillSwitchVerified":false,"sourceKillSwitchVerified":false,"scheduleDisableVerified":false,"transportDisableVerified":false,"outstandingTasksStopVerified":false,"writerClaimReleaseVerified":false}'::jsonb
  ) THEN
    RAISE EXCEPTION 'refusing to roll back Sprout canary activation or evidence state';
  END IF;
END
$$;
...
ALTER TABLE public.career_sources
  DROP CONSTRAINT career_sources_canary_evidence_guard,
  DROP CONSTRAINT career_sources_rollback_evidence_guard,
  DROP COLUMN canary_enabled,
  DROP COLUMN canary_evidence,
  DROP COLUMN rollback_evidence;
