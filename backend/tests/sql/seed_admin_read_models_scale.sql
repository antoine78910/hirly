...
INSERT INTO public.users(user_id,email,name,created_at,data)
SELECT 'scale-user-'||lpad(g::text,6,'0'),
  CASE WHEN g=42 THEN 'selective-user@example.com' ELSE 'candidate-'||g||'@example.com' END,
  'Scale Candidate '||g,timestamptz '2024-01-01'+g*interval '1 second',
...
INSERT INTO public.applications(
 application_id,user_id,job_id,status,package_status,submission_status,created_at,updated_at,data)
SELECT 'scale-app-'||lpad(g::text,7,'0'),
 'scale-user-'||lpad((((g-1)%100200)+1)::text,6,'0'),
 'scale-job-'||lpad((((g-1)%10000)+1)::text,5,'0'),NULL,
 CASE WHEN g%3=0 THEN 'generated' ELSE 'not_generated' END,
 (ARRAY['ready','prepared','submitted','failed','blocked','blocked_captcha',
...
INSERT INTO public.swipes(swipe_id,user_id,direction,created_at,data)
SELECT 'scale-swipe-'||lpad(g::text,7,'0'),
 'scale-user-'||lpad((((g-1)%100200)+1)::text,6,'0'),
 CASE WHEN g%2=0 THEN 'right' ELSE 'left' END,
 timestamptz '2025-06-01'+g*interval '1 second','{}'
FROM generate_series(1,5000000) g;
...
SELECT a.application_id,a.user_id,u.email,a.job_id,a.submission_status,
 CASE WHEN a.submission_status='submitted' THEN 'submitted'
      WHEN a.data->>'manual_status'='offer_expired' THEN 'expired'
      WHEN a.data->>'auto_apply_queue_status'='queued' THEN 'pending'
      ELSE a.submission_status END,
 a.package_status,a.data->>'manual_status',a.created_at,a.updated_at,a.updated_at,
 a.data->>'auto_apply_queue_status','[]',a.updated_at
FROM public.applications a JOIN public.users u USING(user_id);

CREATE TEMP TABLE scale_app_stats AS
SELECT user_id,count(*) total,max(updated_at) last_at FROM public.applications GROUP BY user_id;
CREATE TEMP TABLE scale_swipe_stats AS
SELECT user_id,count(*) total,count(*) FILTER(WHERE direction='right') rights,
 count(*) FILTER(WHERE direction='left') lefts,max(created_at) last_at
FROM public.swipes GROUP BY user_id;
CREATE TEMP TABLE scale_event_stats AS
SELECT user_id,max(created_at) last_at FROM public.analytics_events GROUP BY user_id;

INSERT INTO public.admin_user_read_model(
 user_id,email,name,user_created_at,subscription_status,is_premium,target_location,target_role,
 onboarding_answers,total_applications,total_swipes,right_swipes,left_swipes,
 last_application_at,last_swipe_at,last_event_at,last_active_at,search_text,source_changed_at)
SELECT u.user_id,u.email,u.name,u.created_at,
 ...
 p.data->>'target_location',p.data->>'target_role',p.data#>'{extras,onboarding}',
 a.total,s.total,s.rights,s.lefts,a.last_at,s.last_at,e.last_at,
 GREATEST(a.last_at,s.last_at,e.last_at,u.created_at),
 lower(concat_ws(' ',u.user_id,u.email,u.name,p.data->>'target_location',
   p.data->>'target_role',p.data#>>'{extras,onboarding,job_search_status}',
   p.data#>>'{extras,onboarding,categories}',p.data#>>'{extras,onboarding,selected_roles}')),
 GREATEST(a.last_at,s.last_at,e.last_at,u.created_at)
